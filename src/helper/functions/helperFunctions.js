...
export const findVote = (votes, id) =>
  R.reduce(
    (acc, item) =>
...
//     , { index: -1, cnt: 0 })(votes)
export const findVoteLength = R.reduce(
  (acc, item) => acc.concat(R.length(item)),
  [],
)

export const getMembers = arr =>
  R.reduce((acc, item) => acc.concat(item), [])(arr)

export const getChartData = (votes, choices) =>
  R.compose(
